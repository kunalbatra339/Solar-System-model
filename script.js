document.addEventListener('DOMContentLoaded', () => {
    const bodies = [
        ...
        { name: 'Earth', color: '#4169E1', size: 16, orbitRadius: 140, revolution: 1, rotation: 1, moons: [
            { name: 'Moon', color: '#D3D3D3', size: 5, orbitRadius: 12, revolution: 27.3 }
        ]},
    ...
    asteroidBelt.style.width = `300px`;
    asteroidBelt.style.height = `300px`;
    ...
    bodies.forEach(body => {
        ...
        // Create planetary orbit container
        const planetContainer = document.createElement('div');
        planetContainer.className = 'orbit-container';
        planetContainer.style.width = `${body.orbitRadius * 2}px`;
        planetContainer.style.height = `${body.orbitRadius * 2}px`;
        planetContainer.style.animation = `revolution ${body.revolution * 10}s linear infinite`;
        celestialBodies.appendChild(planetContainer);
        ...
        if (body.name === 'Earth' && body.moons) {
            body.moons.forEach(moon => {
                const moonContainer = document.createElement('div');
                moonContainer.className = 'moon-container';
                moonContainer.style.width = `${moon.orbitRadius * 2}px`;
                moonContainer.style.height = `${moon.orbitRadius * 2}px`;
                ...
                moonElement.style.transform = `translateX(${moon.orbitRadius}px)`;
